import { z } from 'zod'
import { runExecution } from '@/lib/auditor/execute'

export const runtime = 'nodejs'
export const maxDuration = 300

const requestSchema = z.object({ code: z.string().trim().min(1).max(20000), runId: z.string().uuid() }).strict()
...
async function readBoundedBody(request: Request) {
  ...
  try {
    while (true) {
      ...
      if (bytes > 40000) { await reader.cancel(); throw new RangeError('Body too large') }
...
export async function POST(request: Request) {
  ...
  if (Number(request.headers.get('content-length') || 0) > 40000) return Response.json({ error: 'Request too large.' }, { status: 413 })
  let body: unknown
  ...
  if (!parsed.success) return Response.json({ error: 'Invalid execution request. Provide a Python script of 1–20,000 characters.' }, { status: 400 })
  ...
  const stream = new ReadableStream<Uint8Array>({
    async start(controller) {
      ...
      try {
        await runExecution({ code: parsed.data.code, signal, emit(event) { if (!closed && !request.signal.aborted) controller.enqueue(encoder.encode(JSON.stringify({ runId: parsed.data.runId, event }) + '\n')) } })
      } finally {
